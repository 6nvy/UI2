...
function Material.Load(Config)
    ...
    function TabLibrary.ToggleUI(bool)
        bool = type(bool) == 'boolean' and bool or not NewInstance.Enabled
        NewInstance.Enabled = bool
    end
    ...
    function TabLibrary.New(TabConfig)
        ...
        function OptionLibrary.Keybind(kC)
            kC.Text = tostring(kC.Text)
            kC.Key = typeof(kC.Key) == 'EnumItem' and kC.Key or typeof(kC.Key) == 'string' and Enum.KeyCode[kC.Key] or nil
            kC.Callback = typeof(kC.Callback) == 'function' and kC.Callback or function() end
            kC.newCallback = typeof(kC.newCallback) == 'function' and kC.newCallback or function() end
            kC.Menu = typeof(kC.Menu) == 'table' and kC.Menu or {}

            local body = Objects.new('Round')
            body.Name = 'body'
            body.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,30)
            body.ImageColor3 = Theme.Button
            body.ImageTransparency = 0
            body.Parent = PageContentFrame

            local menuAdded

            local function instance(a,b,c)
                local d = Instance.new(a)
                for a,v in next, b or {} do
                    d[a] = v
                end
                for a,v in next, c or {} do
                    v.Parent = d
                end
                return d
            end
            ...
            local textBg = Objects.new('Round')
            textBg.Size = UDim2.new(0,50,1,-10)
            textBg.Position = UDim2.new(1,not menuAdded and -55 or -92,0,5)
            textBg.ImageColor3 = Theme.ToggleAccent
            textBg.Parent = body

            local function ts(a,b,c)
                b[2] = Enum.EasingStyle[ b[2] ]
                game:service('TweenService'):create(a,TweenInfo.new(unpack(b)),c):Play()
            end

            local text = Objects.new('Label')

            text:GetPropertyChangedSignal('Text'):connect(function()
                local sizeOffset = game:service('TextService'):GetTextSize(text.Text,text.TextSize,text.Font,Vector2.new(math.huge,0))
                local newSize,newPos = UDim2.new(0,sizeOffset.X + 10,1,-10),UDim2.new(1,-(sizeOffset.X + (not menuAdded and 15 or 37)),0,5)
                ts(textBg,{0.3,'Sine'},{
                    Size = newSize,
                    Position = newPos
                })
            ...
            local function formatEnum(enum)
                if tostring(enum) == 'nil' then
                    return '...'
                end

                local new = tostring(enum):split('.')[3] 
                return new
            end

            text.Font = Enum.Font.GothamSemibold
            text.TextSize = 14
            text.TextColor3 = Theme.Toggle
            ...
            uis.InputBegan:connect(function(k,t)
                ...
                if k.KeyCode == key and text.Text ~= 'None' then
                    kC.Callback()
                end
            end)
            ...
            menuAdded = TryAddMenu(body, kC.Menu, {})
            delay(0.2,function()
                text.Text = formatEnum(kC.Key)
            end)
            ...
            button.MouseButton1Up:connect(function()
                ...
                inputConnection = uis.InputBegan:connect(function(k,t)
                    local function disallow()
                        inputConnection:Disconnect()
                    end
                    if t or formatEnum(k.KeyCode) == 'Unknown' then
                        disallow()
                        text.Text = 'None'
                        return 
                    end
                    ...
                    key = k.KeyCode
                    local newText = formatEnum(k.KeyCode)
                    text.Text = newText
                    kC.newCallback(k.KeyCode)
                    disallow()
                end)
            end)
        end
